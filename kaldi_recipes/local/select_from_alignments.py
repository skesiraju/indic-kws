...
def load_info_from_align_files(align_files, word_det, legend):

    for align_file in align_files:
        with open(align_file, "r", encoding="utf-8") as fpr:
...
def select_words(words, det, ixs, cur_target_num, args):
    ...
    if len(miss_ixs) == 0:
        print("No words in miss category for the given threshold", args.thresh)
