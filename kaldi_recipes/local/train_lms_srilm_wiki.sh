...
if [ $# -ne 1 ]; then
    echo "USAGE: $0 <data/srilm_wiki/>"
    echo "where: data/srilm_wiki/ is the dir where train.txt is the concatenation of original train.txt and clean wiki.txt"
    echo "       it should also contain dev.txt and vocab files"
    exit;
fi
...
for fname in train.txt dev.txt vocab; do
    if [ ! -f ${tgtdir}/${fname} ]; then
        echo ${tgtdir}/${fname}" not found."
        exit;
    fi
done
