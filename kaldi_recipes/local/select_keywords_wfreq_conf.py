...
def main():
    ...
    test_conf_words = load_keywords(args.conf_dist_file)
    print("test conf words   :", len(test_conf_words))

    wfreq_words = set(test_wf_bin_dict[args.wfreq])

    # print(len(wlen_words), len(wfreq_words))

    common_words = test_conf_words & wfreq_words
...
def parse_arguments():
    ...
    parser.add_argument("conf_dist_file", help="path to conf dist file")
